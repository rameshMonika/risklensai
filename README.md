...
A user asks, in plain language, why their equities portfolio got riskier: *"how concentrated am I?"*, *"what's NVDA's 30-day volatility?"*, *"why did NVDA fall this week?"*, or *"investigate my portfolio risk"*. The system classifies the question and runs only the agents that question needs. Every risk number is computed in deterministic Python, never by the LLM. For a full investigation it also retrieves market and news evidence for the biggest risk driver. A simple quantitative question gets a single plain sentence back. A full investigation gets a cited report with an explicit limitations section.

The scope is deliberately narrow. It covers a portfolio of about 5 to 10 equities and no cash balance, so portfolio value is the sum of holding market values. There is no trading, optimization, price prediction, or personalized recommendation. Three tempting features were specified and then deferred past V1 rather than half-built: beta and benchmark comparison, VaR, and the Sharpe ratio. V1 has no benchmark data source, and each of the three needs its own methodology decision first.

The core design rule is that the LLM chooses *which* deterministic function to call and interprets its result. It never does the arithmetic itself. `INVESTIGATION.risk_results` holds the machine-readable calculation output. `INVESTIGATION_REPORT.report_json` holds the LLM's narrative over it. The first is the ground truth the pytest suite checks against. The second is interpretation. [How the investigation pipeline works](#how-the-investigation-pipeline-works) shows how this is proven out rather than just asserted.
...
| Public API | **core-service**, FastAPI (`:8000`) | Auth, portfolio and holdings CRUD, investigation orchestration, and all persistence. The only internet-facing service. |
| Agent API | **agent-service**, FastAPI (`:8100`) | The LangGraph investigation pipeline. Fully stateless, no database, internal-only. |
...
| Intent routing | `semantic-router` + `sentence-transformers` (MiniLM `HuggingFaceEncoder`) | Local, no-API embedding of the question, then per-intent KMeans centroids and a plain argmax over 5 intents. Settled winner over an LLM-prompt supervisor (see [Evaluation results](#evaluation-results)). |
...
| Migrations | Alembic | `alembic upgrade head` on core-service startup. Bootstraps a fresh Postgres, and is a no-op once at head. |
...
| Evaluation | In-notebook harness | 5 dimensions (answer correctness, agent selection, trajectory, latency, safety and reliability) over 33 labelled questions, plus LLM-as-judge for open-ended reports |
...
*Diagram 4. Azure topology. Solid arrows are request traffic, dashed arrows are supporting wiring.*
...
The router carries no refusal path of its own. It always returns one of the five intents, and the guardrail upstream is what blocks anything off-topic or unsafe. Calibration only fits the centroids, and its accuracy is measured as argmax-matches-label (100% on the calibration set).

### Agents and MCP tool servers

Market, Risk, and News are real MCP servers: separate Python subprocesses over stdio. They are spawned once at agent-service startup (`mcp_manager`) and then reused, since spawning a process per call would be far too slow.

| Agent | Tool server | What it does |
|---|---|---|
| Portfolio | none | Reshapes the holdings snapshot already in `state["holdings"]`. It shapes state, it does not fetch data. |
| Market | Alpha Vantage MCP (`get_historical_prices`, `get_company_overview`) | Prices and company metadata. It does not compute returns (Risk's job) or fetch benchmark prices (beta is deferred). Falls back to deterministic synthetic data when the free tier is exhausted. |
| Risk | local deterministic MCP (`calculate_symbol_risk`, `run_full_investigation`) | Wraps the pure functions in `finance.py`. |
| News | Tavily MCP (`search_news(symbol, company_name, days, max_results)`) | Always given a specific symbol and window, never a blind multi-holding search. |

### Deterministic risk calculations, the LLM never computes a metric

Every number comes from a pure Python function in `finance.py`.

| Function | What it computes | Why it matters |
|---|---|---|
| `calculate_returns` | Simple daily returns, `(P_t − P_{t-1}) / P_{t-1}`, from a close-price series. | The input to volatility and correlation. Working in returns rather than raw prices makes holdings with very different share prices comparable. |
| `calculate_volatility` | Annualised volatility: sample standard deviation of daily returns times `√252`. | The headline "how bumpy is this holding" number. Annualising puts a 30-day and a 90-day window on the same scale. |
| `calculate_max_drawdown` | Largest peak-to-trough decline over the period, as a negative decimal. | Shows the worst loss an investor holding through the period would have sat through, which volatility alone hides. |
| `calculate_concentration` | Market value and portfolio weight per holding, plus the single largest weight. | Answers "am I too concentrated". A portfolio that is 60% one stock is risky however calm that stock looks. |
| `calculate_sector_exposure` | Each holding's weight summed into its sector. | Catches hidden concentration: five different names that are all Technology still move together. |
| `calculate_correlation_matrix` | Pairwise Pearson correlation of daily returns across holdings, aligned by date. | Diversification only helps when holdings do not move together. High pairwise correlation means the portfolio is less diversified than the holding count suggests. |
| `calculate_contribution_to_loss` | Each holding's share of the total portfolio value change over the period. | Ranks the risk drivers. The top contributor is the symbol the full-investigation route hands to News to explain. |

None of these use a model, the network, or randomness. The Risk node calls them over MCP and writes the results into `state["risk_results"]`. The Answer and Report nodes only read that dict, they never compute a number themselves.

`calculate_beta`, `calculate_var`, and `calculate_sharpe_ratio` are deferred. V1 has no benchmark price source, and each of the three needs its own methodology decision first.
