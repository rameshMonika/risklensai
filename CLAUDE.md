...
- A small local encoder (`HuggingFaceEncoder`, no API key) embeds each question; per-intent KMeans cluster centroids (a handful of representative vectors per intent, not every raw example) represent each business intent (holdings, concentration, symbol_risk, news_reason, full_investigation). The router always picks the single highest-scoring intent (plain argmax across the 5) — it has no refusal path of its own; see the resolved-bug note below for why.
- A **separate, dedicated safety/relevance guardrail** (a small fast Groq LLM call, structured-output classification) runs *ahead of* the intent router and blocks the pipeline entirely — in blocking mode, so no agent executes — on two independent grounds, both returned by the same call: `unsafe` (prompt-injection / instruction-override attempts) and `off_topic` (a genuine, benign question with nothing to do with portfolio risk, e.g. weather, jokes). Each gets its own refusal message. The router force-matches to its closest of 5 known intents no matter how unrelated the input is (see below), so the guardrail is the *only* place either kind of refusal actually happens — not a belt-and-suspenders layer on top of router-level refusal.

This decision resolves the "Not yet decided" item that used to track this; no hybrid was needed.

**Resolved bug (found and fixed during the V1 build): the router originally had its own calibrated-threshold refusal path, and its calibration was broken.** Coordinate-ascent threshold search over a labelled calibration set — including a `"None"` bucket of off-topic/injection examples the router was meant to refuse on its own — reliably converged to all-zero thresholds (`{'holdings': 0.0, 'concentration': 0.0, 'symbol_risk': 0.0, 'news_reason': 0.0, 'full_investigation': 0.0}`, "None cases correctly refused: 0/10"), meaning the router never actually refused anything; it force-matched every question, however unrelated, to whichever of the 5 intents scored highest. This was never caught by the 33-case Phase 2 eval suite, which has no benign-off-topic test cases (only normal routes, unknown-ticker reliability cases, and prompt-injection safety cases — the latter were still correctly blocked by the guardrail regardless of this bug), so the 99.4%/100% Safe/Reliable numbers never actually exercised the broken path. Root cause: real score separation does exist in the data (e.g. `symbol_risk`'s genuine matches scored 0.599+ vs off-topic examples topping out at 0.266), but the coordinate-ascent search only ever tried candidate thresholds equal to an observed score, never a value strictly between two clusters, so it could never find the separating gap. **Fix:** rather than repairing that search, the router's refusal responsibility was removed entirely and merged into the guardrail (the `unsafe`/`off_topic` split described above) — the router became a plain unconditional argmax over the 5 intents, calibration now only fits KMeans centroids (no thresholds, no `"None"` bucket), and calibration accuracy is now measured as plain argmax-matches-label rate (100% on the calibration set) instead of weighted threshold accuracy. This is simpler than the original threshold-search design and removes the failure mode entirely rather than patching around it.
