...
**Known bug (found during the V1 build, unresolved): the router's calibrated thresholds are all 0.0, so the router never actually refuses anything.** Cell 34 of `02_agent_evaluation_v3.ipynb` (the coordinate-ascent threshold search) produces `{'holdings': 0.0, 'concentration': 0.0, 'symbol_risk': 0.0, 'news_reason': 0.0, 'full_investigation': 0.0}` with "None (refusal) cases correctly refused: 0/10" — this is the notebook's own saved output, not a regression introduced while building `agent_service`. It was never caught because the 33-case Phase 2 eval suite has no benign-off-topic test cases (only normal routes, unknown-ticker reliability cases, and prompt-injection safety cases, which the separate Groq guardrail catches regardless of this bug) — so the 99.4%/100% Safe/Reliable numbers above never actually exercised this path. Practical effect: a genuinely off-topic question ("what's the weather?") is *not* refused by the router as this section claims — it gets force-matched to whichever of the 5 intents scores highest (even at a low, meaningless score) and run through that intent's real pipeline. Prompt-injection attempts are unaffected and still correctly blocked by the guardrail. Root cause looks fixable (real score separation exists in the calibration data, e.g. `symbol_risk`'s genuine matches score 0.599+ vs off-topic examples topping out at 0.266 — the coordinate-ascent search just never tries threshold values that fall *between* observed scores, only values equal to one). Fix not yet chosen as of this note — options on the table: (a) fix the coordinate-ascent candidate-threshold generation, (b) replace it with independent per-intent threshold calibration (simpler, no cross-route interaction), or (c) widen the guardrail's own job to also judge topical relevance (not just injection-attempt detection) and let it absorb this refusal responsibility instead, making the router's own threshold moot.
